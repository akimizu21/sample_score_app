...
def import_masters():
    """
    マスターデータをCSVファイルからインポート
    リクエストボディに以下のフィールドを含む multipart/form-data で送信:
    - subjects_file: 科目マスタCSV
    - universities_file: 大学マスタCSV
    - faculties_file: 学部マスタCSV
    """
    result = {"universities": 0, "faculties": 0, "subjects": 0, "errors": []}

    # --- 大学マスタの取り込み ---
    if 'universities_file' in request.files:
        try:
            file = request.files['universities_file']
            # メモリ上で読み込み
            content = file.read().decode('utf-8-sig')
            df_uni = pd.read_csv(StringIO(content)).dropna(subset=["university_code", "university_name"])
            
            existing_uni_ids = {u.university_id for u in Universities.query.all()}
            new_uni = []
            for _, row in df_uni.iterrows():
                try:
                    code = int(row["university_code"])
                    ...
                    result["errors"].append(f"大学データエラー: {e}")
                    continue
            if new_uni:
            ...
            result["errors"].append(f"大学マスタ取込エラー: {str(e)}")

    # --- 学部マスタの取り込み ---
    if 'faculties_file' in request.files:
        try:
            file = request.files['faculties_file']
            content = file.read().decode('utf-8-sig')
            df_fac = pd.read_csv(StringIO(content)).dropna(subset=["faculty_code", "faculty_name", "university_code"])
            ...
            for _, row in df_fac.iterrows():
                try:
                    fac_code = int(row["faculty_code"])
                    uni_code = int(row["university_code"])
                    ...
                    result["errors"].append(f"学部データエラー: {e}")
                    continue
            if new_unis:
            ...
            result["errors"].append(f"学部マスタ取込エラー: {str(e)}")

    # --- 科目マスタの取り込み ---
    if 'subjects_file' in request.files:
        try:
            file = request.files['subjects_file']
            content = file.read().decode('utf-8-sig')
            df_subj = pd.read_csv(StringIO(content)).dropna(subset=["subject_code", "subject_name"])
            ...
            for _, row in df_subj.iterrows():
                try:
                    ...
                    result["errors"].append(f"科目データエラー: {e}")
                    continue
            if new_subjects:
            ...
            result["errors"].append(f"科目マスタ取込エラー: {str(e)}")
